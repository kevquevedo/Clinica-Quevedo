...
@Component({
  ...
  styleUrls: ['./grafico-solicitados.component.css']
})
export class GraficoSolicitadosComponent {
  titulo : string = 'Meses';
  informacion1!: any[];
  informacion2!: any[];
  nombre1!: string;
  ...
  descargarPDF(){
    let element = document.querySelector("#grafico2") as HTMLElement;
    html2canvas(element).then((canvas: any) => {
      ...
      pdfFile.text( "TURNOS SOLICITADOS POR MEDICO", 120, 130);
      let imgData = canvas.toDataURL("image/jpeg", 2.5);
      pdfFile.addImage(imgData, 0, 150, 600, 200);
      pdfFile.save('Turnos-Solicitados-Por-Medico.pdf');
    });
  }
}
